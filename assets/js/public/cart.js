// ================ ENHANCED GUEST CART & CHECKOUT SYSTEM ================
      ...
      document.addEventListener("DOMContentLoaded", async function () {
        ...
        const isLoggedIn =
          userId && sessionStorage.getItem("isLoggedIn") === "true";
      ...
      async function fetchPricingConfig() {
        try {
          ...
          showNotification(
            "Using default pricing. " + error.message,
            "warning"
          );
    ...
    `;
      ...
      `;
            ...
            item.complementaryItems.forEach((complementary, compIndex) => {
              ...
              if (
                complementary.size &&
                typeof complementary.size === "number"
              ) {
                compPrice = (complementary.price * complementary.size).toFixed(
                  2
                );
      ...
      function updateCartSummary() {
        ...
        const pricingConfig = JSON.parse(
          sessionStorage.getItem("pricingConfig")
        ) || {
          vatPercentage: 10,
      ...
      function updateQuantity(index, change, newValue = null) {
        ...
        if (cart[index]) {
          ...
          if (isLoggedIn && cart[index].cartItemId) {
            ...
            pushCartItemToServer(userInfo.userId, cart[index]).catch(
              (error) => {
  ...
  `;
      ...
      function saveNotes(itemIndex, compIndex) {
        ...
        if (
          ...
          cart[itemIndex].complementaryItems[compIndex].notes =
