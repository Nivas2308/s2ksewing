...
document.addEventListener("DOMContentLoaded", async function () {
 ...
 document.addEventListener("DOMContentLoaded", async function () {
   ...
   try {
     ...
     showNotification(
       "Error loading checkout page. Please try again.",
       "error"
     );
     showLoading(false);
   }
...
function updateCheckoutUI() {
       ...
       item.complementaryItems.forEach((comp) => {
         ...
         summaryHTML += `
           <div class="summary-item complementary-summary-item">
             <img src="${comp.image || "https://via.placeholder.com/60"}" alt="${
           comp.title
 ...
 async function placeOrder() {
   try {
     ...
     const isCardPayment = !document
       .getElementById("codPayment")?.classList.contains("active");
     ...
     showLoading(true);
           ...
           if (
             document.getElementById("codPayment")?.classList.contains("active")
           ) {
             paymentMethod = "cod";
           } else {
   ...
   if (isLoggedIn && userInfo.userId) {
     ...
     const scriptUrl = "https://script.google.com/macros/s/AKfycbwZAY2t1QhVdNqX8Wsd1gTu1j7-ChXF-xpydhZzzaGcpI29chwKt5e0IxCS8uTdiHk/exec";
 ...
 function updatePaymentMethodAvailability() {
   ...
   if (!onlinePaymentEnabled) {
     ...
     if (!infoMsg) {
       ...
       infoMsg.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Online payment is temporarily unavailable due to a technical issue. Please choose another payment method or try again later.';
       const paymentSection = document.querySelector('.payment-methods');
       if (paymentSection) paymentSection.parentNode.insertBefore(infoMsg, paymentSection.nextSibling);
     }
 ...
 function updateCODChargesRow(show) {
   ...
   if (show) {
     ...
     if (!codRow) {
       ...
       codRow.innerHTML = '<span class="summary-label">COD Charges</span><span class="summary-value" id="codChargesValue"></span>';
     ...
     document.getElementById('codChargesValue').textContent = `$${codCharges.toFixed(2)}`;
   } else {
 ...
 window.calculateOrderSummary = function() {
   ...
   const codSelected = document.getElementById('codPayment')?.classList.contains('active');
