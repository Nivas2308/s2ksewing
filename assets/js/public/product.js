...
const API_URL = "https://script.google.com/macros/s/AKfycbxJSOLkj6RSe2pC0fZA9SA7sAZQpPZvNqzpNEtBxXhGMegnSYx65ieVJ2VeOBon4ks5/exec";
...
function checkUserReviewStatus() {
    ...
    console.log("Checking review status for user:", currentUser, "product:", productId);
    
    const script = document.createElement('script');
    const callbackName = 'userStatusCallback_' + Math.floor(Math.random() * 1000000);
    ...
    script.src = `${API_URL}?action=checkUserReview&productId=${encodeURIComponent(productId)}&userId=${encodeURIComponent(currentUser)}&callback=${callbackName}`;
...
function showUserReviewStatus(userReview) {
    ...
    const reviewForm = document.querySelector('.review-form') || document.querySelector('.review-form-container');
    const reviewsSection = document.querySelector('#reviews-container').parentNode;
            ...
            <div class="user-existing-review">
                ...
                <div class="your-review">
                    <div class="review-header">
                        <span class="reviewer-name">${escapeHTML(userReview.name || "You")}</span>
                        <span class="review-date">${dateString}</span>
                    </div>
                    <div class="rating">
                        ${"★".repeat(userReview.rating)}${"☆".repeat(5 - userReview.rating)}
                    </div>
                    <div class="review-comment">${escapeHTML(userReview.comment || "")}</div>
                </div>
            </div>
        `;
    } else {
...
function showRatingError(message) {
    const starContainer = document.querySelector('.stars') || document.querySelector('.rating');
    if (starContainer) {
        const errorDiv = document.createElement('div');
...
function loadReviews() {
    ...
    window[callbackName] = function(data) {
        ...
        if (data && data.success && data.reviews && data.reviews.length > 0) {
            ...
            console.log(`Found ${data.reviews.length} reviews for product ID: ${productId}`);
            ...
            reviewCountInfo.textContent = `Showing ${Math.min(3, reviews.length)} of ${reviews.length} review${reviews.length !== 1 ? 's' : ''}`;
    ...
    const timeout = setTimeout(() => {
        if (window[callbackName]) {
            container.innerHTML = '<p class="error">Failed to load reviews. Please try refreshing the page.</p>';
            delete window[callbackName];
        }
    }, 10000);
    
    script.id = 'jsonp-script';
    script.src = `${API_URL}?action=getReviews&productId=${encodeURIComponent(productId)}&callback=${callbackName}`;
    script.onerror = function() {
...
function calculateAverageRating(reviews) {
    ...
    const sum = reviews.reduce((total, review) => total + (parseInt(review.rating) || 0), 0);
...
function displayReviewSummary(reviews) {
    ...
    if (!summaryContainer) {
        ...
        reviewsContainer.parentNode.insertBefore(summaryContainer, reviewsContainer);
    }
...
function resetUserId() {
    if (confirm("This will reset your user ID and allow you to review products again. Are you sure?")) {
...
function renderProductPage(product) {
 ...
 // Update the DOM
  ...
  if (product.category && product.category.toLowerCase() === "sewing patterns") {
    const fabricsBtn = document.getElementById("selectFabricsBtn");
    if (fabricsBtn) {
      fabricsBtn.innerHTML = "Select Fabrics <span class='required-indicator'>*</span>";
      ...
      if (categoryOptions) {
        ...
        requiredNote.innerHTML = "<span class='required-indicator'>*</span> Fabric selection is required for this pattern";
...
function filterProducts() {
  ...
  filteredProducts = complementaryProducts.filter((p) => {
    ...
    const matchesSubcategory =
      ...
      (p.subCategory &&
        p.subCategory.toLowerCase() === subcategoryFilter);
          ...
          ${
            item.category && item.category.toLowerCase() === "fabrics"
              ? `<div class="edit-field">
                  ...
                  <input type="number" id="editSize" min="0.5" step="0.5" value="${item.size || 1}">
                </div>`
              : ""
          }
          <div class="edit-field">
...
function previewItem(itemId) {
  ...
  const images = item.images || [item.mainImage || item.image || "https://via.placeholder.com/300"];
      ...
      <div class="preview-popup-content">
        ...
        <div class="preview-body">
          <div class="preview-images">
            <img src="${images[0]}" alt="${item.title}" class="preview-main-image">
            ${images.length > 1 ? 
              `<div class="preview-thumbnails">
                ${images.map((img, index) => 
                  `<img src="${img}" alt="Preview ${index+1}" class="preview-thumbnail" 
          ...
          <div class="preview-details">
            <div class="preview-category">
              ${item.category ? `<span class="preview-tag">${item.category}</span>` : ''}
              ${item.subCategory ? `<span class="preview-tag">${item.subCategory}</span>` : ''}
            </div>
            
            <div class="preview-price">${priceDisplay}</div>
            
            ${item.inStock !== undefined ? 
              `<div class="preview-stock ${item.inStock ? 'in-stock' : 'out-of-stock'}">
  ...
  if (popupType === "pattern") {
    ...
    if (patternsButton) {
      const patternCount = selectedComplementaryItems.filter(
        item => item.category && item.category.toLowerCase() === "sewing patterns"
      ).length;
...
function addToGuestCart(cartItem) {
  ...
  const originalBtnText = addToCartBtn ? addToCartBtn.textContent : "Add to Cart";
  if (addToCartBtn) {
  ...
  try {
    ...
    for (let i = 0; i < guestCart.length; i++) {
      ...
      if (existingItem.id === cartItem.id) {
        const sameOptions = areOptionsEqual(existingItem.options, cartItem.options);
      ...
      for (let i = 0; i < serverCart.length; i++) {
        ...
        if (existingItem.id === cartItem.id) {
          ...
          if (sameOptions && sameComplementary) {
            ...
            return updateCartItemOnServer(existingItem.cartItemId, updatedItem, userId);
          }
        }
      }
...
function updateCartItemOnServer(cartItemId, updatedItem, userId) {
  ...
  // API endpoint URL
  const apiUrl = "https://script.google.com/macros/s/AKfycbxrQrGtswF2tkxMjtWGqWgCH7C0ewQuoMKb2YVfTc5FVe0kdsQoYjwToq_PiVbtxEQ/exec";
  // Login Detail API 
...
function saveCartToServer(cartItem, userId) {
  ...
  .then(data => {
    if (data.success) {
      ...
      throw new Error(data.message || "Unknown server error when adding to cart");
    }
  });
}
      ...
      if (data.success) {
        ...
        return cartItems.map(item => {
          ...
          return {
            ...
            complementaryItems: Array.isArray(item.complementaryItems) ? item.complementaryItems : []
          };
        });
      } else {
  ...
  return loadCartFromServer(userId)
    .then(serverCart => {
      ...
      const syncPromises = guestCart.map((guestItem) => {
        ...
        const existingItemIndex = mergedCart.findIndex(serverItem => {
          return (
            ...
            areComplementaryItemsEqual(serverItem.complementaryItems, guestItem.complementaryItems)
          );
        });
        ...
        if (existingItemIndex >= 0) {
          ...
          updatedItem.quantity = (parseInt(updatedItem.quantity) || 1) + (parseInt(guestItem.quantity) || 1);
          
          // Update on server and in merged cart
          mergedCart[existingItemIndex] = updatedItem;
          return updateCartItemOnServer(updatedItem.cartItemId, updatedItem, userId);
        } else {
      ...
      return Promise.all(syncPromises).then(() => {
        ...
        localStorage.removeItem("guestCart");
        console.log("Cart synced successfully, total items:", mergedCart.length);
        return true;
      });
    })
    .catch(error => {
...
function loadRecommendedProducts(allProducts, currentProduct) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => initRecommended(allProducts, currentProduct));
  } else {
...
function initRecommended(allProducts, currentProduct) {
  ...
  if (currentProduct.category) {
    const sameCategoryProducts = allProducts.filter(product => 
      ...
      product.category && 
      product.category.toLowerCase().trim() === currentProduct.category.toLowerCase().trim()
    );
    ...
    if (sameCategoryProducts.length > 0) {
      ...
      console.log('Recommended products categories:', recommendedProducts.map(p => `${p.category} - ${p.subCategory || 'No subcategory'}`));
    } else {
...
function renderRecommendedProducts(recommendedProducts) {
  ...
  if (recommendedProducts.length === 0) {
    recommendedContainer.innerHTML = '<p>No similar products available in this category.</p>';
    return;
  }
  ...
  recommendedProducts.forEach(product => {
    const productImage = product.images 
      ? product.images[0] 
      : product.mainImage || product.image || "https://via.placeholder.com/220x180";
      
    html += `
      <div class="recommended-product-card">
        <a href="productpage.html?id=${product.id}" class="product-link">
          <img src="${productImage}" alt="${product.title}" class="recommended-product-img">
          <div class="recommended-product-body">
            ...
            <div class="recommended-product-category">${product.category}${product.subCategory ? ` - ${product.subCategory}` : ''}</div>
...
function initializeSlider(productCount) {
  ...
  if (slider && prevBtn && nextBtn) {
    ...
    nextBtn.addEventListener('click', () => {
      ...
      const maxPosition = -(Math.max(0, productCount - visibleItems) * cardWidth);
      position -= cardWidth * visibleItems;
