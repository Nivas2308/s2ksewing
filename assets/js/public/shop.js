...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    ...
    if (urlCategory) {
      ...
      const formattedCategory = urlCategory.replace(/\b\w/g, (c) =>
        c.toUpperCase()
      );
      // Wait for products to be loaded
      if (urlSubCategory) {
        // Capitalize first letter of each word for subcategory
        const formattedSubCategory = urlSubCategory.replace(/\b\w/g, (c) =>
          c.toUpperCase()
        );
  ...
  paginatedProducts.forEach((product, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="card-footer">
        ...
        <button class="card-button" onclick="event.stopPropagation(); redirectToProduct(${
          product.id
        })">Buy</button>
      </div>
    `;
...
async function loadCategories() {
  ...
  sortedCategories.forEach((category) => {
    ...
    categoryCard.innerHTML = `
      ...
      <select id="${category.replace(
        /\s+/g,
        ""
      )}SubCategory" onchange="filterProducts('${category}')">
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", function () {
      ...
      document
        .querySelectorAll("#categorySidebar select")
