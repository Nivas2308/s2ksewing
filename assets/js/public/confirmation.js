...
async function fetchOrderDetails(orderNumber) {
  try {
    ...
    if (DEBUG_MODE) {
      document.getElementById(
        "debugText"
      ).innerHTML += `<br>API Response: ${JSON.stringify(result, null, 2)}`;
    }
...
function displayOrderDetails(order) {
  if (DEBUG_MODE) {
    document.getElementById(
      "debugText"
    ).innerHTML += `<br>Displaying order: ${JSON.stringify(order, null, 2)}`;
  }

  // Handle different possible field names from your Google Apps Script
  const orderNumber = order["Order ID"] || order.id || order.orderId || "N/A";
  const orderDate = order["Date"] || order.date || order.orderDate || "N/A";
  const customerName =
    order["Customer Name"] ||
    (order.customer
  ...
  const orderTotal =
    order["Total"] || (order.order ? order.order.total : order.total) || "N/A";
